...
def find_nearest_group(group, i):
    ...
    while prev_idx >= 0 or next_idx < len(group):
        if prev_idx >= 0 and group.iloc[prev_idx]['Létszám'] >= 10:
            closest_idx = prev_idx if closest_idx is None else (prev_idx if group.iloc[prev_idx]['Létszám'] < group.iloc[closest_idx]['Létszám'] else closest_idx)
        if next_idx < len(group) and group.iloc[next_idx]['Létszám'] >= 10:
            closest_idx = next_idx if closest_idx is None else (next_idx if group.iloc[next_idx]['Létszám'] < group.iloc[closest_idx]['Létszám'] else closest_idx)
    ...
    for code in grouped['KépzésKód'].unique():
        ...
        for i in range(len(group)):
            if group.iloc[i]['Létszám'] < 10:
                ...
                original_index = modified_data[(modified_data['KépzésKód'] == code) & (modified_data['Évfolyam'] == current_year)].index
